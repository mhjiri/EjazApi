...
{
    public class ListOfSuggestedBooks
    {
        public class Query : IRequest<Result<PagedList<SuggestBookQryDto>>>
        {
            public BookParams Params { get; set; }
        }

        public class Handler : IRequestHandler<Query, Result<PagedList<SuggestBookQryDto>>>
        {
            ...
            public async Task<Result<PagedList<SuggestBookQryDto>>> Handle(Query req, CancellationToken cancellationToken)
            {
                var qry = _ctx.SugggestBook
                    .ProjectTo<SuggestBookQryDto>(_mpr.ConfigurationProvider)
                    .AsQueryable();
                ...
                return Result<PagedList<SuggestBookQryDto>>
                    .Success(await PagedList<SuggestBookQryDto>.CreateAsync(qry,
