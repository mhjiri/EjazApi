...
namespace Application.Groups
{
    public class Activate
    {
        ...
        public class Handler : IRequestHandler<Command, Result<Unit>>
        {
            ...
            {
                var group = await _ctx.Groups.FindAsync(new object[] { req.Id }, cancellationToken: cancellationToken);
                ...
                group.Gr_Active = true;

                var result = await _ctx.SaveChangesAsync(cancellationToken) > 0;

                if (!result) return Result<Unit>.Failure("Failed to activate Group");
