    ...
    public class UpdateCustomer
    {
        ...
        public class Handler : IRequestHandler<Command, Result<ProfileQryDto>>
        {
            ...
            public async Task<Result<ProfileQryDto>> Handle(Command req, CancellationToken cancellationToken)
            {
                ...
                {
                   return Result<ProfileQryDto>.Failure("Failed to update Customer - Email already  taken!");
                }

                if (await _ctx.Users.AnyAsync(x => x.PhoneNumber == req.Profile.PhoneNumber && x.UserName != profile.UserName))
                {
                    return Result<ProfileQryDto>.Failure("Failed to update Customer - Phone Number already taken!");
                }
                ...
                if (!result) return Result<ProfileQryDto>.Failure("Failed to update Customer");
                ...
                return Result<ProfileQryDto>.Success(_mpr.Map<ProfileQryDto>(req.Profile));
            }
        }
    }
}