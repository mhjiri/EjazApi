...
namespace Application.BookCollections
{
    public class ListByAuthor
    {
        ...
        public class Handler : IRequestHandler<Query, Result<PagedList<BookCollectionDto>>>
        {
            ...
            {
                List<Guid> authorBooks = _ctx.Books
                    .Where(s => s.Authors.Select(z => z.At_ID).Contains(req.Id)).Select(s => s.Bk_ID).ToList();

                var qry = _ctx.BookCollections.Where(s => s.Books.Where(b => b.BkBc_Active && authorBooks.Contains(b.Bk_ID)).Count() > 0)
